...
//文字列描画メソッド
void CDrawFont::StrCenterDraw(const wchar_t* str, float x, float y, float size, float color[4])
{
	list<unique_ptr<CFontCharTex>>::iterator itr;	//登録文字検索用
	bool hit;										//登録ヒットしているかチェック
	...
	y -= size / 2.0f;
	Font::StrDraw(str, x, y, size, color);
}

void  CDrawFont::StrDraw(const wchar_t* str, float x, float y, float size, COLOR_A color)
{
	float col[4] = { color.r,color.g,color.b,color.a };
	Font::StrDraw(str, x, y, size, col);
}
void  CDrawFont::StrRightDraw(const wchar_t* str, float x, float y, float size, COLOR_A color)
{
	float col[4] = { color.r,color.g,color.b,color.a };
	...
	for (unsigned int i = 0; i < wcslen(str); i++)
	{
		...
		char mc[3];
		int len;
		wctomb_s(&len, mc, sizeof(wchar_t), str[i]);
		if (len == 1)
		{
			x -= size / 2.0f;
		}
...
void  CDrawFont::StrCenterDraw(const wchar_t* str, float x, float y, float size, COLOR_A color)
{
	float col[4] = { color.r,color.g,color.b,color.a };
	Font::StrCenterDraw(str, x, y , size, col);
}
