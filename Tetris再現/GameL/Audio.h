...
namespace GameL
{
	...
	typedef class CAudio
	{
		public:
			...
			static void LoadAudio2(int id, const char* name, SOUND_TYPE type);	//サウンドボイスデータ登録
			static void DeleteAudio();					//サウンド情報破棄
			static void Start(int id,float sec = 0.0f);					//スタート	
			static void Stop(int id);					//ストップ
			static void StopAudio();					//ストップ
		//	static void DemoStart(int id, float sec);
